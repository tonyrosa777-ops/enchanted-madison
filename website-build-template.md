...
export default function HomePage() {
  return (
    <>
      ...
      <FounderStory />      {/* 3. Trust */}
      <Philosophy />        {/* 4. Methodology / Why */}
      <Services />          {/* 5. What you offer */}
      <ShopPreview />       {/* 6. Products (if applicable) */}
      <Stats />             {/* 7. Social proof — numbers */}
      <Testimonials />      {/* 8. Social proof — humans */}
      <InstagramFeed />     {/* 9. Live / current (optional) */}
      <QuizCTA />           {/* 10. Engagement hook */}
      <BlogPreview />       {/* 11. Authority content */}
      <FinalCTA />          {/* 12. Last chance CTA */}
    </>
  );
}
```
...
- [ ] `STRIPE_SECRET_KEY` — from Stripe → Developers → API keys (use live key in production)
- [ ] `STRIPE_WEBHOOK_SECRET` — from Stripe → Webhooks → your endpoint → Signing secret
- [ ] `RESEND_API_KEY` — from Resend → API Keys (after verifying your sending domain)
- [ ] `NEXT_PUBLIC_SITE_URL` — canonical domain with protocol and www if applicable (e.g. `https://www.yourdomain.com`)
