...
You are the lead architect on a high-stakes website build for **The Enchanted Collective** (enchantedmadison.com) — a luxury glamping and romantic experience property based in Madison, Indiana. This is a full ground-up build: new brand identity, conversion-optimized architecture, built on the Optimus standard stack.
...
| Variable | Fill with |
|----------|-----------|
| `[BUSINESS_NAME]` | The Enchanted Collective |
| `[DOMAIN]` | enchantedmadison.com |
| `[BUSINESS_TYPE]` | luxury glamping and romantic experience property |
| `[LOCATION]` | Madison, Indiana |
| `[LAUNCH_TARGET]` | June 2026 |
| `[PRIMARY_AUDIENCE]` | romantic couples aged 27–45 within 2-hour drive of Louisville, Cincinnati, and Indianapolis |
| `[CORE_OFFER]` | glamping tents, private cottage, day-use hot tub escapes, proposal packages |
| `[KEY_GOAL]` | drive direct bookings and own the romantic getaway category in southern Indiana |
| `[BOOKING_ENGINE]` | Lodgify (overnight stays) + Acuity Scheduling (experiences) |
...
- [ ] Sections to remove identified (shop, blog, quiz, Instagram — per client needs)
...
Before running any commands, confirm both input files are complete:
...
If either file is incomplete, halt and flag before proceeding.
...
```bash
npx create-next-app@latest enchanted-madison \
  --typescript \
  --tailwind \
  --eslint \
  --app \
...
6. Create `vercel.json` at the **repo root**:
   ```json
   { "rootDirectory": "enchanted-madison" }
   ```
...
**Primary objective:**
Drive direct bookings and own the romantic getaway category in southern Indiana.

**Target audience:**
Romantic couples aged 27–45 within 2-hour drive of Louisville, Cincinnati, and Indianapolis.

**Core offerings:**
Glamping tents, private cottage, day-use hot tub escapes, proposal packages.
